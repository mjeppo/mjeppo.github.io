...
export default function PokeCard(props) {
  ...
  const imgList = Object.keys(sprites || {}).filter((val) => {
    ...
    const cryList = Object.keys(cries || {}).filter((val) => {
        if (!cries[val]) {
            console.log("geen sound");
            return false;
            
        }
       if (["legacy", "other"].includes(val)) {
         return false;
       }
       return true
    })
  ...
  useEffect(() => {
    ...
    // check if selectedPokemon info is avaliable in cache
    // 1. define cache
    let cache = {};
    ...
    // data is not in cache so we have to fetch it from api
    ...
    // 3. is we fetch from api save info to cache
  }, [selectedPokemon]);
  ...
  return (
    <div className="poke-card">
      ...
      <div className="audio-container">
        {cryList.map((cryUrl, cryIndex) => {
          const oggUrl = cries[cryUrl];
          return <audio controls key={cryIndex} src={oggUrl} alt={`${name}-img-${cryUrl}`} />;
        })}
          </div>
